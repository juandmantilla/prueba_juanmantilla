...
@Component
public class FranquiciaRepositorioAdaptador extends ReactiveAdapterOperations<Franquicia, FranquiciaEntidad, Integer, FranquiciaRepository> implements FranquiciaPuerto {

    private static final Logger log = LoggerFactory.getLogger(FranquiciaRepositorioAdaptador.class);

    public FranquiciaRepositorioAdaptador(FranquiciaRepository repository, ObjectMapper mapper) {
    ...
    public Mono<Franquicia> agregarFranquicia(Franquicia franquicia) {
        ...
        return repository.save(mapper.map(franquicia, FranquiciaEntidad.class))
                .map(this::toEntity)
                .onErrorResume(e -> {
                            log.error("Error al ejecutar el guardado de la Franquicia", e);
                            return Mono.error(new PruebaExcepcion("No se pudo guardar la entidad Franquicia:", e));
                        }
                );
    }
    ...
    public Mono<Franquicia> actualizarNombreFranquicia(String nombreFranquicia, Integer id) {
        ...
        return repository.findById(id)
                ...
                .onErrorResume(e -> {
                            ...
                            return Mono.error(new PruebaExcepcion("No se pudo actualizar la entidad Franquicia:", e));
                        }
                );
    }
}
